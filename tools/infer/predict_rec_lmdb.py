...
def main(args):
    ...
    txn = lmdb.open(args.image_dir, readonly=True).begin()
    ...
    try:
        ...
        preds = []
        batch = []
        for key, value in tqdm(txn.cursor(), total=txn.stat()['entries']):
            if key.decode().split("-")[0] != 'image':
                continue

            name = key.decode().split("-")[1]

            image = cv2.imdecode(np.frombuffer(value, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
            label = txn.get(f"label-{name}".encode()).decode()
            size = txn.get(f"size-{name}".encode()).decode()

            write_name = "{:09d}".format(max_name)
            if len(batch) < args.rec_batch_num:
                batch.append(image)
            else:
                res, _ = text_recognizer(batch)
                for i in range(len(batch)):
                    preds.append([write_name, res[i][0], res[i][1]])
                    max_name += 1
                batch = []
            # res, _ = text_recognizer([image])
            # preds.append([write_name, res[0][0], res[0][1]])
            max_name += 1
        for pred in preds:
            if args.save_log_path is not None:
                with open(args.save_log_path, "a+") as fout:
                    fout.write(pred[0] + "\t" + pred[1] + "\t" + str(pred[2]) + "\n")
    ...
    #     if args.save_log_path is not None:
    #         with open(args.save_log_path, "a+") as fout:
    #             fout.write(valid_image_file_list[ino].rsplit('/',1)[-1] + "\t" + rec_res[ino][0] + "\t" + str(rec_res[ino][1]) +
    #                        "\n")
